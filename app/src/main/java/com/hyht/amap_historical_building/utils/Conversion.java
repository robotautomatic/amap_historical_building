...
public class Conversion {
    ...
    private static final int NOR_SOUTH_EAST_ZONE = 49;
...
//	private static double sm_EccSquared = 6.69437999013e-03;
     ...
     */
    private LatLng MapLatLonToXY(double phi, double lambda, double lambda0 ) {
     ...
     */
    public LatLng MapXYToLatLon( double x, double y, double lambda0 ) {
        ...
        return new LatLng( philambda[0], philambda[1] );
    }
     ...
     */
    public LatLng LatLonToUTMXY( double lat, double lon, int zone ) {
        double[] xy = new double[2];
        lat = DegToRad( lat );
        lon = DegToRad( lon );
        LatLng point = MapLatLonToXY( lat, lon, UTMCentralMeridian( zone ) );

        /* Adjust easting and northing for UTM system. */
        LatLng nPoin = new LatLng(point.latitude * UTMScaleFactor + 500000.0,point.longitude * UTMScaleFactor);
        if ( xy[1] < 0.0 )
    ...
    public LatLng UTMXYToLatLon( double x, double y ) {
     ...
     * Returns: The function does not return a value.
     */
    public LatLng UTMXYToLatLon( double x, double y, int zone, boolean isOnSouthernHemisphere ) {
        double cmeridian;
